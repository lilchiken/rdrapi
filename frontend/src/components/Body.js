...
export default function Body() {
  ...
  const sendRequest = () => {
    axios(`http://localhost:8000/api/${searchInputValue}`)
      .then((r) => {
        setError("");
  ...
  const sendEmail = () => {
    axios.post('http://localhost:8000/api/subEmail', {email: resultEmail})
      .then((r) => {
  ...
  return (
    <main className="main">
      <div className="FAQ">
        ...
        <div className="List">
          • Python 3, FastAPI & Uvicorn, Celery, alembic & SQLAlchemy <p></p>
          • JavaScript, React, CSS <p></p>
          • Nginx, Docker, docker-compose, Redis, PostgreSQL
        </div>
        <div className="Ask">
